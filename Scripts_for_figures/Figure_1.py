...
def extract_cross_section(
    ...
    with h5py.File(h5file, "r") as f:
        ...
        fig_section = plt.figure(figsize=(12, 6),constrained_layout=True)
        ax_section = plt.axes()

        x_vals = lon_pts if xaxis == "lon" else lat_pts
        xlabel = "Longitude (°)" if xaxis == "lon" else "Latitude (°)"
        xmin = np.floor(x_vals.min() * 5) / 5     # 1 / 0.2 = 5
        xmax = np.ceil(x_vals.max() * 5) / 5
        ax_section.set_xlim(xmin, xmax)
        ymin = np.floor(z_km.min() * 2) / 2       # 1 / 0.5 = 2
        ymax = np.ceil(z_km.max() * 2) / 2
        ax_section.set_ylim(ymax, ymin)

        ax_section.set_xlabel(xlabel,fontsize=16)
        ax_section.set_ylabel("Depth (km)",fontsize=16)
        ax_section.set_ylim([z_km.max(), z_km.min()])
        ax_section.tick_params(axis="x", direction="in", top=True, which="major",labelsize=13)
        ax_section.tick_params(axis="y", direction="in", right=True, which="major",labelsize=13)
        ax_section.yaxis.set_major_locator(MultipleLocator(0.5))
        ax_section.yaxis.set_major_formatter(FormatStrFormatter('%.1f'))
        ax_section.set_xlim(x_vals.min(), x_vals.max())

        # Longitude axis: 0.2° spacing, 1 decimal
        ax_section.xaxis.set_major_locator(MultipleLocator(0.2))
        ax_section.xaxis.set_major_formatter(FormatStrFormatter('%.1f'))
        ax_section.minorticks_off()
        # Cross-section end labels A and A'
        #ax_section.text(
         #   0.0, 1.02, "A",
       ...
       # ax_section.text(
        ...
        cbar = fig_section.colorbar(pcm, ax=ax_section)
        cbar.set_label(r"$V_S$ (km/s)", fontsize=16)
        cbar.ax.tick_params(labelsize=13)
        # --- Annotated region numbers (data coordinates) ---
        labels = {
            "1": (175.55, 2.0),   # (longitude, depth)
            "2": (175.05, 2.0),
            "3": (175.50, 0.1),
            "4": (174.62, 2.3),
        }
        ...
        if png:
            ...
            fig_section.savefig(section_filename, dpi=900)
